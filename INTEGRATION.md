...
## 架构:你在复用什么
项目分两层(重构后):
- **`injenium.core`** —— 域无关的经验能力市场内核:链/合约、市场技能(共 13 个:只读自检/浏览 + 悬赏闭环 + 技能货架 + 运行状态/中止)、内容寻址
...
> - dimOS 配置使用 blueprint 长选项，例如 `--marketskillcontainer.chain-backend mock`。
>   用 `dimos run injenium.market --help` 查看当前版本支持的完整参数。
...
$(dirname $DPY)/dimos mcp list-tools                  # 应看到 13 个技能(含运行状态/中止)
$(dirname $DPY)/dimos mcp call chain_status           # 只读自检:钱包/余额/链是否可达(不花钱)
$(dirname $DPY)/dimos mcp call list_requests          # 只读浏览:板上开放请求(不花钱)
$(dirname $DPY)/dimos mcp call publish_request --json-args '{"need":"test","budget":"1.0"}'
$(dirname $DPY)/dimos stop
```
能列出 13 个技能、调用返回 str,即“技能契约”打通。
...
$(dirname $DPY)/dimos run injenium.agentic -d \
  --marketskillcontainer.chain-backend mock \
  --marketskillcontainer.market-state-path /shared/market_state.json \
  --marketskillcontainer.memory-db /data/go2_recording.db \
  --requestlistener.chain-backend mock \
  --requestlistener.market-state-path /shared/market_state.json \
  --requestlistener.poll-interval 5
```
...
$(dirname $DPY)/dimos mcp call publish_request --json-args '{"need":"climb the ramp","budget":"1.0"}'
...
$(dirname $DPY)/dimos mcp call run_status --arg run_id=offer:<id>   # 必须等到 state=succeeded
$(dirname $DPY)/dimos mcp call pay --arg offer_id=<id>
$(dirname $DPY)/dimos stop
```
> `fetch_and_run` 在真机上通过 `Go2Primitives` 执行白名单原语(移动/导航/跟随/sport/wait);
> 沙箱绝不 eval 对方代码。运行中、失败或已中止时 `pay` 会拒绝放款。
...
$(dirname $DPY)/dimos run injenium.agentic -d \
  ...
  --marketskillcontainer.memory-db /data/go2_recording.db \
  --marketskillcontainer.recipe-storage local \
...
| `marketskillcontainer.max_transaction_inj` | Agent 单笔挂牌/托管/购买金额上限 | `0.1` |
| `marketskillcontainer.min_gas_reserve_inj` | 真实链转账后必须保留的 gas 余额 | `0.01` |
| `tx_receipt_timeout` | 回执/区块确认总等待秒数 | `180` |
| `tx_confirmations` | 交易成功后要求的区块确认数 | `1` |
| `pending_tx_path` | 已签名待确认交易的恢复文件 | `injenium_artifacts/pending_txs.json` |
| `tx_recovery_scan_blocks` | 重启后最多回查的区块数 | `2048` |
...
> 命令行字段使用短横线，例如配置字段 `market_state_path` 对应
> `--marketskillcontainer.market-state-path`。`match_tags`/`match_keywords`(列表)留空=匹配所有开放请求;
> 需过滤则用 `dimos run ... -c <config.json>` 传 JSON。
...
- **Injective 回执索引延迟**:客户端会在回执查询之外扫描区块中的 sender+nonce,
  并从事件/链状态恢复新 ID。若最终超时,先用 Blockscout 和账户 nonce 核对,
  不要盲目用新 nonce 重发同一业务操作。
- **进程在广播后退出**:签名交易会以 `0600` 权限写入 `pending_tx_path`;
  重启后的下一次写操作先确认或原样重播它。`chain_status` 可只读查看待恢复 nonce/hash。
